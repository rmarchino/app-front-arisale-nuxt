  ...
  <v-card>
    ...
    <v-app-bar color="#FDFEFE">
      ...
      <template v-slot:extension>
        ...
        <v-card>
          <v-btn color="deep-purple accent-4" text class="button__actualizar">
                ...
                <template v-slot:item="props">
                  <tr class="custom-row">
                    <td class="custom-cell">
                      {{ props.item.issueDate | formatIssueDate }}
                    </td>
                    <td class="custom-cell">
                      {{ props.item.duration | formatDuration }}
                    </td>
                    <td class="custom-cell">
                      {{ props.item.establishmentName }}
                    </td>
                    ...
                    <td class="custom-cell">
                      {{ props.item.startDate | formatStartDate }}
                    </td>
                    <td class="custom-cell">
                      {{ props.item.endDate | formatEndDate }}
                    </td>
                    <td>
                      <SeeMore :item="props.item" />
                    </td>
                  </tr>
                </template>
              </v-data-table>
          ...
          <v-row style="background-color: #a6acaf">
            <v-col cols="12" class="container__button-flex">
              <v-btn
                ...
                >Previous</v-btn
              >
              <v-btn color="indigo" class="button__next" @click="nextPage"
                >Next</v-btn
              >
              <v-select
                v-model="perPage"
  ...
  methods: {
    ...
    async changePerPage() {
      try {
        ...
        console.log(
          'Error al cambiar la cantidad de elementos por página:',
          error
        )
      }
    },
